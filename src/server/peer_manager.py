...
from src.types.peer_info import peer_info
...
class ExtendedPeerInfo:
    ...
    def get_new_bucket(self, nKey, src_peer=self.src):
        hash1 = int.from_bytes(
            bytes(
                std_hash(
                    nKey
...
class AddressManager:
    def __init__(self):
        self.id_count = 0
        self.nKey = secrets.randbits(256)
        self.random_pos = []
        self.tried_matrix = [
            [
    ...
    def create_(self, addr: PeerInfo, addr_src: PeerInfo):
        ...
        return (map_info[node_id], node_id)
    ...
    def make_tried_(self, info, node_id):
        ...
        self.count_new -= 1
        cur_bucket = info.GetTriedBucket(self.nKey)
        cur_bucket_pos = info.get_bucket_position(self.nKey, False, new_bucket)
        if self.tried_matrix[cur_bucket][cur_bucket_pos] != -1:
            #Evict the old node from the tried table.
            int node_id_evict = self.tried_matrix[new_bucket][new_bucket_pos]
            assert node_id_evict in self.map_info
            old_info = self.map_info[node_id_evict]
            old_info.is_tried = False
            self.tried_matrix[new_bucket][new_bucket_pos] = -1
            self.tried_count -= 1
            #Find its position into new table.
            new_bucket = old_info.get_new_bucket(self.nKey)
            new_bucket_pos = old_info.get_bucket_position(self.nKey, true, nUBucket)
    ...
    def mark_good_(self, addr, test_before_evict, nTime):
        ...
        for n in range(NEW_BUCKET_COUNT):
            ...
            int nBpos = info.get_bucket_position(self.nKey, True, nB)
    ...
    def attempt_(self, addr, count_failures, nTime):
        ...
        info.nLastTry = nTime
        if (fCountFailure and info.nLastCountAttempt < info.nLastGood):
        ...
        if (
            ...
            while True:
                ...
                while self.tried_matrix[nKBucket][nKBucketPos] == -1:
                    nKBucket = (nKBucket + secrets.randbits(math.log2(TRIED_BUCKET_COUNT))) % TRIED_BUCKET_COUNT
                    nKBucketPos = (nKBucketPos + secrets.randbits(math.log2(BUCKET_SIZE))) % BUCKET_SIZE
                node_id = self.tried_matrix[nKBucket][nKBucketPos]
                (info, _) = self.map_info[node_id]
                if secrets.randbits(30) < (fChanceFactor * info.GetChance() * (1 << 30)):
                    return info
                fChanceFactor *= 1.2
        else:
            fChanceFactor = 1.0
            while True:
                ...
                nUBucketPos = randrange(ADDRMAN_BUCKET_SIZE)
                while self.new_table[nUBucket][nUBucketPos] == -1:
                    nUBucket = (nUBucket + secrets.randbits(math.log2(NEW_BUCKET_COUNT))) % NEW_BUCKET_COUNT
                    nUBucketPos = (nUBucketPos + secrets.randbits(math.log2(BUCKET_SIZE))) % BUCKET_SIZE
    ...
    def select_tried_collision_(self):
        if len(self.tried_collisions) == 0
            return None
        new_id = random.choice(self.tried_collisions)
        ...
        tried_bucket_pos = newInfo.get_bucket_position(self.nKey, False, tried_bucket)
    ...
    def get_addr_(self):
        ...
        for n in range(len(self.random_pos)):
            ...
            int nRndPos = randrange(len(self.random_pos) - n) + n
